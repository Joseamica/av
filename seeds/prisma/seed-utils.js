...
exports.cleanDatabase =
  exports.createDeliverect =
  exports.createAvailabilities =
  exports.createEmployees =
  exports.createProductsAndModifiers =
...
async function createCategories(menuId, branchId) {
  ...
  return Promise.all(
    [
      ...
      db_server_1.prisma.menuCategory.create({
        data: {
          name,
...
async function createProductsAndModifiers(categories, branchId) {
  ...
  await Promise.all(
    [
      ...
      db_server_1.prisma.modifiers.create({
        data: {
          name,
          extraPrice,
          modifierGroupId: modifierGroup.id,
  ...
  return Promise.all(
    categories.flatMap((category, i) =>
      range(1, 2).map(j =>
        db_server_1.prisma.menuItem.create({
          data: {
