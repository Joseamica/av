...
const Backdrop = ({ children, handleClose }: { children: JSX.Element; handleClose: () => void }) => (
  <motion.div
...
const ModalContent = ({
  ...
  <>
    <motion.div
      ...
      } no-scrollbar inset-x-0  bottom-0 m-auto flex w-full max-w-md flex-col items-center overflow-hidden ${className || 'bg-background dark:bg-DARK_0 rounded-t-2xl     '}`}
      variants={effect}
      initial="hidden"
      animate="visible"
      exit="exit"
    ...
    >
      ...
      {handleClose && !imgHeader && (
        <div className="flex w-full">
          <FlexRow className={`dark:bg-mainDark w-full items-center justify-between bg-white drop-shadow-md    ${imgHeader ? null : 'p-4 shadow-inner'}`}>
            {/* {backButton ? null : ( */}
...
export const ModalFullScreen = ({ children, className, isOpen, handleClose, hideCloseButton, backdropDismiss = true, onExitComplete, ariaLabel, imgHeader, title }: ModalProps) => {
  const [isBrowser, setIsBrowser] = useState(false)
  const [trigger, setTrigger] = onExitComplete ?? [undefined, undefined]
        ...
        <Backdrop handleClose={backdropDismiss ? handleClose : undefined}>
          <FocusLock>
            <motion.div
              ...
              aria-modal={true}
              aria-label={ariaLabel}
              className={`fixed z-50 m-auto flex justify-center ${className || 'no-scrollbar  inset-0 h-full  w-screen max-w-md   items-center overflow-scroll'}`}
              variants={effect}
              initial="hidden"
              animate="visible"
              exit="exit"
              onClick={event => event.stopPropagation()}
            >
              <div className="dark:bg-DARK_0 m-5 w-full rounded-xl bg-white p-5">{children}</div>
            </motion.div>
          </FocusLock>
        </Backdrop>
      )}
