...
export async function loader({request, params}: LoaderArgs) {
  ...
  const amountLeft = (await getAmountLeftToPay(tableId)) || null
...
export async function action({request, params}: ActionArgs) {
  ...
  // const cartItems = JSON.parse(formData.getAll('cartItems'))
  ...
  if (paymentMethod === 'card') {
    ...
    await assignExpirationAndValuesToOrder(amountToPay, tip, amountToPay, order)
...
export function CartPayment() {
  ...
  const matchData = matches.find(
    match => match.id === 'routes/table.$tableId_.menu.$menuId.cart',
  ).data
  // console.log('matchData', matchData)
  const cartItems = matchData?.cartItems
  // console.log('cartItems', cartItems)
  ...
  const total = matchData?.cartItemsTotal
  const tip = Number(data.total) * (Number(tipRadio) / 100)
  ...
  return (
    <>
      <div className="dark:bg-night-bg_principal dark:text-night-text_principal sticky inset-x-0 bottom-0 flex flex-col justify-center rounded-t-xl border-4 bg-day-bg_principal px-3">
        ...
        <AnimatePresence initial={false}>
          {total > 0 && (
            ...
            >
              <FlexRow justify="between">
                <H5>Quieres pagar ({matchData.cartItems.length} platillos):</H5>
                <Underline>
      ...
      <input
        type="hidden"
        name="amountToPay"
        value={matchData?.cartItemsTotal}
      />
    </>
  )
}
