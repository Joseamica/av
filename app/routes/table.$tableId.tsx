...
export default function Table() {
   ...
   const [selectedUsers, setSelectedUsers] =
      useState<string[]>([])
   const [filterPerUser, setFilterPerUser] =
      useState(false)
...
export async function loader({
   ...
   invariant(
      tableId,
      'No se encontró el ID de la mesa',
   )

   const branch = await getBranch(tableId)
   invariant(branch, 'No se encontró la sucursal')

   const [table, usersInTable] =
      await Promise.all([
         getTable(tableId),
   ...
   if (userId && username) {
      ...
      if (!isUserInTable) {
         try {
            console.log(
               `🔌 Connecting '${username}' to the table`,
            )
            ...
            console.log(
               `✅ Connected '${username}' to the table`,
            )
         } catch (error) {
            console.log(
            ...
            throw new Error(
               `No se pudo conectar al usuario con la mesa ${error}`,
            )
         }
      }
      const isUserInOrder =
      ...
      if (!isUserInOrder && order) {
         try {
            console.log(
               `🔌 Connecting '${username}' to the order`,
            )
            ...
            EVENTS.ISSUE_CHANGED(tableId)
            console.log(
               `✅ Connected '${username}' to the order`,
            )
         } catch (error) {
            console.log(
               '%cerror table.$tableId.tsx line:361 ',
            ...
            throw new Error(
               `No se pudo conectar al usuario con la orden ${error}`,
            )
         }
      }
   }
   ...
   if (order) {
      ...
      amountLeft = await getAmountLeftToPay(
         tableId,
      )
